...
const BLOCK_NAME = `${PLUGIN_NAME}/crawler` // modifier
...
registerBlockType(BLOCK_NAME, {
  title: __('Categorie & description'),
  ...
  save: ({ attributes: { imageUrl, title, subtitle, category, textLink, link } }) => (
    <div className='presentation crawler'>
      <div>
        <h3>{category}</h3>
        <h2>{title}</h2>
        <p>{subtitle}</p>
        <div>
          <a className='seafoam-button' href={link}>{textLink}</a>
        </div>
      </div>
      <img src={imageUrl} alt='' />
    </div>
